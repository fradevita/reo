program reconstruction

  use grid_2d
  use volume_of_fluid
  use mpi

  implicit none

  integer :: i, j, np, s, smax
  real(kind=dp) :: Lx, Ly, delta, r
  real(kind=dp) :: x, y, xc, yc, dt
  type(field) :: u(1), v(1)

  ! Initialize MPI
  call MPI_INIT(ierr)
  call MPI_COMM_RANK(MPI_COMM_WORLD, pid, ierr)
  call MPI_COMM_SIZE(MPI_COMM_WORLD, num_procs, ierr)
  mpi_common_world = MPI_COMM_WORLD

  np = 50
  Lx = pi/2.
  Ly = pi/2.
  delta = Lx / np
  r = 0.2*pi
  xc = 0.5*pi
  yc = 0.2*(pi + 1)

  allocate(boxarray(1))
  if (pid == 0) then
    call create_box(1, np, np, Lx, Ly, 0.d0, 0.d0)
    boxarray(1)%right_boundary = 'halo'
    boxarray(1)%rp = 1
    boxarray(1)%top_boundary = 'halo'
    boxarray(1)%tp = 2
  endif
  if (pid == 1) then
    call create_box(1, np, np, Lx, Ly, Lx, 0.d0)
    boxarray(1)%left_boundary = 'halo'
    boxarray(1)%lp = 0
    boxarray(1)%top_boundary = 'halo'
    boxarray(1)%tp = 3
  endif
  if (pid == 2) then
    call create_box(1, np, np, Lx, Ly, 0.d0, Ly)
    boxarray(1)%right_boundary = 'halo'
    boxarray(1)%rp = 3
    boxarray(1)%bottom_boundary = 'halo'
    boxarray(1)%bp = 0
  endif
  if (pid == 3) then
    call create_box(1, np, np, Lx, Ly, Lx, Ly)
    boxarray(1)%left_boundary = 'halo'
    boxarray(1)%lp = 2
    boxarray(1)%bottom_boundary = 'halo'
    boxarray(1)%bp = 1
  endif
  
  call init_vof()
  call set_vof('circle', xc, yc, r)

  ! Define velocity field
  allocate(u(1)%f(0:np+1,0:np))
  allocate(v(1)%f(0:np,0:np+1))
  do j = 0,np
    y = boxarray(1)%p0(2) + (j - 0.5)*delta
    do i = 0,np+1
      x = boxarray(1)%p0(1) + (i - 1.)*delta
      u(1)%f(i,j) = sin(x)*cos(y)
    end do
  end do

  do j = 0,np+1
    y = boxarray(1)%p0(2) + (j - 1.)*delta
    do i = 0,np
      x = boxarray(1)%p0(1) + (i - 0.5)*delta
      v(1)%f(i,j) = -cos(x)*sin(y)
    end do
  end do

  ! Timestep
  dt = 0.0025*pi

  ! Number of steps
  smax = int(10*pi/dt)

  ! Print the interface at t = 0
  !open(unit = 1, file = 'vof')
  do j = 1,np
    y = boxarray(1)%p0(2) + (j - 0.5)*delta
    do i = 1,np
      x = boxarray(1)%p0(1) + (i - 1.)*delta
      write(10+pid,*) x, y, vof(1)%f(i,j)
    end do
    write(10+pid,*) ''
  end do
  write(10+pid,*) ''
  write(10+pid,*) ''

  ! Time cycle
  do s = 1,smax

    write(10+pid,*) ''
    ! When t = T/2 reverse the velocity field and save the profile 
    if (s == smax/2) then
      do j = 0,np
        y = boxarray(1)%p0(2) + (j - 0.5)*delta
        do i = 0,np+1
          x = boxarray(1)%p0(1) + (i - 1)*delta
          u(1)%f(i,j) = -sin(x)*cos(y)
        end do
      end do
      do j = 0,np+1
        y = boxarray(1)%p0(2) + (j - 1)*delta
        do i = 0,np
          x = boxarray(1)%p0(1) + (i - 0.5)*delta
          v(1)%f(i,j) = cos(x)*sin(y)
        end do
      end do

      ! Print the interface at t = T/2
      do j = 1,np
        y = boxarray(1)%p0(2) + (j-0.5)*delta
        do i = 1,np
          x = boxarray(1)%p0(1) + (i-0.5)*delta
          write(10+pid,*) x, y, vof(1)%f(i,j)
        end do
        write(10+pid,*) ''
      end do
      write(10+pid,*) ''
      write(10+pid,*) ''
    endif 

    call advect_vof(u, v, dt)
  end do

  ! Print the interface at t = T
  do j = 1,np
    y = boxarray(1)%p0(2) + (j-0.5)*delta
    do i = 1,np
      x = boxarray(1)%p0(1) + (i-0.5)*delta
      write(10+pid,*) x, y, vof(1)%f(i,j)
    end do
    write(10+pid,*) ''
  end do
  write(10+pid,*) ''
  write(10+pid,*) ''
  close(10+pid)

  ! Finalize the simulation
  call MPI_finalize(ierr)

end program reconstruction
