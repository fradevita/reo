program poisson_dirichlet
  ...
  do n = 3,8
    ...
    ny = 2**n / num_procs
    Lx = 1.0
    Ly = 1.0 / float(num_procs)
    x0 = 0.
    y0 = 0.

    ! and create the computational grid
    allocate(boxarray(1))
    call create_box(1, nx, ny, Lx, Ly, x0, y0 + Ly*pid)

    ! Set the grid extent
    boxarray(1)%ilower = [1, 1 + ny*pid]
    boxarray(1)%iupper = [nx, ny + ny*pid]
    ...
    ! If in parallel
    if (num_procs > 1) then
      if (pid == 0) boxarray(1)%top = 'halo'
      if (pid == 1) boxarray(1)%bottom = 'halo'
    endif
